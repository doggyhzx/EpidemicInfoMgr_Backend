...
def publish_comment(request):
    ...
    for word in banned_word_list:
        if word in newsInfo:
            return render(response, "发布失败，评论包含非法内容")
    ...
    Comment_id = models.Comment.objects.aggregate(Max('cmt_id'))+1
    Reliable_id = 3
    ...
    # 插入新闻评论关系表
    newsComm_id = models.NewsComments.objects.aggregate(Max('news_cmt_id'))+1
    comm_relation = models.NewsComments.objects.create(
        news_cmt_id=newsComm_id, cmt_id=Comment_id, news_id=News_id)

    # 插入用户评论关系表
    Pub_cmt_id = models.PublishComments.objects.aggregate(Max('pub_cmt_id'))+1
...
def report(request):
    ...
    try:
        ...
        # 修改评论状态
        models.Comment.objects.filter(cmt_id=Cmt_id).update(is_reliable=1)
...
def news_list(request):
    ...
    for news in Allnews:
        ...
        news_item['news_gen_time'] = news.news_gen_time
